...
- 当前 105 个测试，覆盖：chain（8）、opencode（17）、attach（7）、sync（4）、store（6）、graph（6）、ref（6）、list（3）、summarizer（10）、retry（3）、show（4）、paths（4）、reflog（2）、db-reader（2）、data-access（4）、checkout（5）、compact（4）、inject（4）、pick（4）、rebase（2）
